...
class DataReceiveManager
{
    ...
    public DataReceiveManager(String port)
    {
        while (Static_Lock.lockConditionPortManager == true) ;
        Static_Lock.lockConditionPortManager = true;
        IPEndPoint ip = new IPEndPoint(IPAddress.Any, Convert.ToInt32(port));
        Socket socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);

        //Console.WriteLine("Binding--Data");
        socket.Bind(ip);
        Console.WriteLine("Bind successful... Data Manager");
        socket.Listen(10);
        ...
        Static_Lock.lockConditionPortManager = false;
